...
struct boss_halazzi : public BossAI
{
    ...
    void Reset() override
    {
        ...
        if (GameObject* gate = instance->GetGameObject(GO_DOOR_HALAZZI))
        {
            gate->SetGoState(GO_STATE_READY); // 关门
        }
        if (GameObject* gate = instance->GetGameObject(GO_GATE_HEXLORD))
        {
            gate->SetGoState(GO_STATE_READY); // 关门
        }
    }
    ...
    void JustDied(Unit* killer) override
    {
        ...
        if (GameObject* gate = instance->GetGameObject(GO_DOOR_HALAZZI))
        {
            gate->SetGoState(GO_STATE_ACTIVE); // 开门
        }
        if (GameObject* gate = instance->GetGameObject(GO_GATE_HEXLORD))
        {
            gate->SetGoState(GO_STATE_ACTIVE); // 开门
