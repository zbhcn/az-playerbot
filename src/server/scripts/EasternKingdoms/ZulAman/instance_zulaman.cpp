...
public:
    ...
    struct instance_zulaman_InstanceMapScript : public InstanceScript
    {
        ...
        ObjectGuid HexLordGateGUIDEXIT;
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            switch (go->GetEntry())
            {
                ...
                case GO_GATE_HEXLORD_EXIT:
                    HexLordGateGUIDEXIT = go->GetGUID();
                    break;
        ...
        void CheckInstanceStatus()
        {
            ...
            {
                HandleGameObject(HalazziDoorGUID, true);//开老4出口门
                HandleGameObject(HexLordGateGUID, true);//开老5门
            }
            if (BossKilled > DATA_GONGEVENT)
            {
                HandleGameObject(MassiveGateGUID, true);//开门口门
                HandleGameObject(AkilzonDoorGUID, true);
            }
            if (BossKilled > DATA_HEXLORDEVENT)
            {
                HandleGameObject(HexLordGateGUIDEXIT, true);
        ...
        void SetData(uint32 type, uint32 data) override
        {
            switch (type)
            {
                ...
                case DATA_HALAZZIEVENT:
                    ...
                    {
                        HandleGameObject(HalazziDoorGUID, false);
