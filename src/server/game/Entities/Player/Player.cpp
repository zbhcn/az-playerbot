...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!areaEntry)
    {
        ...
        TeleportTo(571, 5813.87, 448.76, 658.75, 2.237);//卡在空中就传回达拉然
        return;
    }
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= nullptr*/, uint32 spellid /*= 1*/)
{
    ...
    //增加鸟点秒飞
    if (HasItemCount(38186, 50, 0) || (sWorld->getIntConfig(CONFIG_INSTANT_TAXI) == 1 || (sWorld->getIntConfig(CONFIG_INSTANT_TAXI) == 2 && m_isInstantFlightOn)) && !spellid)
    {
