...
bool PetAI::CanAttack(Unit* target, SpellInfo const* spellInfo)
{
    ...
    // 检查当前目标的绝对值高度差或是否飞天
    Unit* owner = me->GetCharmerOrOwner();
    if (target && owner && owner->IsClass(CLASS_HUNTER, CLASS_CONTEXT_PET) && !me->CanFly() && !me->isSwimming())
    {
        float zDiff = std::abs(me->GetPositionZ() - target->GetPositionZ());
        if ((me->GetMap() && me->GetMap()->IsDungeon() && target->IsFlying())
            || (target->IsFlying() && zDiff > 3.0f))
        {
            if (me->isMoving())
                me->StopMoving();
            me->AttackStop();
            me->GetMotionMaster()->MoveFollow(owner, PET_FOLLOW_DIST, 1.0f);
            return false;
        }
    }
