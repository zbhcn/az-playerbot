...
public:
    ...
    void OnLogin(Player* pPlayer) override
    {
        if (sConfigMgr->GetOption<bool>("Arena1v1.Announcer", true))
            ChatHandler(pPlayer->GetSession()).SendSysMessage("服务器已开启1v1竞技场.");
    }

    void GetCustomGetArenaTeamId(const Player* player, uint8 slot, uint32& id) const
    {
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "排队进入1v1竞技场（未分级）", GOSSIP_SENDER_MAIN, 20);
        }
        ...
        else
        {
            if (!player->InBattlegroundQueueForBattlegroundQueueType(bgQueueTypeId))
            {
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "排队进入1v1竞技场（分级）", GOSSIP_SENDER_MAIN, 2);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "清除竞技场队伍", GOSSIP_SENDER_MAIN, 5, "你确定吗?", 0, false);
            }

            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "显示您的统计信息", GOSSIP_SENDER_MAIN, 4);
        }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            case 1: // Create new Arenateam
            {
                ...
                {
                    handler.PSendSysMessage("你必须达到等级 %u + 才能创建1v1竞技场队伍", sConfigMgr->GetOption<uint32>("Arena1v1.MinLevel", 70));
                    return true;
                }
            ...
            case 2: // Join Queue Arena (rated)
            {
                if (Arena1v1CheckTalents(player) && !JoinQueueArena(player, creature, true))
                    handler.SendSysMessage("加入队列时出现问题1.");
            ...
            case 20: // Join Queue Arena (unrated)
            {
                if (Arena1v1CheckTalents(player) && !JoinQueueArena(player, creature, false))
                    handler.SendSysMessage("加入队列时出现问题2.");
...
private:
    ...
    bool CreateArenateam(Player* player, Creature* me)
    {
        ...
        ChatHandler(player->GetSession()).SendSysMessage("1v1竞技场队伍创建成功!");
    ...
    bool Arena1v1CheckTalents(Player* player)
    {
        ...
        for (uint32 talentId = 0; talentId < sTalentStore.GetNumRows(); ++talentId)
        {
            ...
            {
                ChatHandler(player->GetSession()).SendSysMessage("你不能加入，因为你有被禁止的天赋。");
                return false;
            }
        ...
        if (count >= 36)
        {
            ChatHandler(player->GetSession()).SendSysMessage("你不能加入，因为你有太多禁用的天赋点。（治疗/坦克）");
            return false;
        }
