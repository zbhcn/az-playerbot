...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    if (!bot->InBattleground() && !bot->inRandomLfgDungeon() && bot->GetGroup() &&
        !bot->GetGroup()->isLFGGroup() && !bot->isUsingLfg())
	{
        ...
        if (leader &&(leader->isAFK() || !leader->GetSession() || GET_PLAYERBOT_AI(leader) ||
            !HasRealPlayerMaster() || !master || !master->IsInWorld()))
        {
            bot->RemoveFromGroup();
            ...
            if (!bot->IsBeingTeleported())
                bot->TeleportTo(571, 5813.87, 448.76, 658.75, 2.237);      // 传送回达拉然
            if (bot->GetMap() && !bot->GetMap()->IsRaidOrHeroicDungeon())  // 重置副本CD
            {
                for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
                {
                    ...
                    for (BoundInstancesMap::const_iterator itr = binds.begin(); itr != binds.end();)
                    {
                        ...
                        {
                            sInstanceSaveMgr->PlayerUnbindInstance(bot->GetGUID(), itr->first, Difficulty(i), true,
                                                                   bot);
                            itr = binds.begin();
                        }
                        else
...
void PlayerbotAI::DoNextAction(bool min)
{
    ...
    if ((!master || (masterBotAI && !masterBotAI->IsRealPlayer())) && group)
    {
        ...
        {
            if (botAI->HasActivePlayerMaster() && bot->GetGroup()->IsLeader(bot->GetGUID()))
                {
                    WorldPacket p(SMSG_GROUP_SET_LEADER, 8);
                    p << master->GetGUID();
                    bot->GetSession()->HandleGroupSetLeaderOpcode(p);
                    botAI->SayToParty("我不熟悉这里,还是让蠢货带路吧!");
                ...
                bot->GetMapId() != lfgleader->GetMapId())
            {
                ...
                bot->TeleportTo(lfgleader->GetMapId(), lfgleader->GetPositionX(), lfgleader->GetPositionY(),
                                lfgleader->GetPositionZ(), 0);
...
ActivePiorityType PlayerbotAI::GetPriorityType(ActivityType activityType)
{
    ...
    if (bot->IsBeingTeleported() ||
        !bot->IsInWorld() ||
        !HasRealPlayers(bot->GetMap()) ||
        !bot->GetMap()->IsGridLoaded(bot->GetPositionX(), bot->GetPositionY()))
...
bool PlayerbotAI::AllowActive(ActivityType activityType)
{
    ...
    if (botActiveAlonePerc <= 0) return false;
    if (botActiveAlonePerc >= 100) return true;
...
uint32 PlayerbotAI::SmartScaleActivity(ActivePiorityType type, uint32 botActiveAlonePerc)
{
    ...
    switch (type)
    {
        ...
        case ActivePiorityType::IN_INSTANCE:
        case ActivePiorityType::VISIBLE_FOR_PLAYER:
            // we always enforce 100% with the above acitivities
            // otherwise it will effect the pve gameplay.
            return 100; 
        case ActivePiorityType::IS_ALWAYS_ACTIVE:
        case ActivePiorityType::IN_COMBAT:
            ActivePiorityTypeMod = 0.9;
            break;
        case ActivePiorityType::IN_BG_QUEUE:
        case ActivePiorityType::IN_LFG:
            ActivePiorityTypeMod = 0.7;
            break;
        case ActivePiorityType::NEARBY_PLAYER:
        case ActivePiorityType::PLAYER_FRIEND:
        case ActivePiorityType::PLAYER_GUILD:
        case ActivePiorityType::IN_ACTIVE_AREA:
            ActivePiorityTypeMod = 0.6;
            break;
        case ActivePiorityType::IN_ACTIVE_MAP:
        case ActivePiorityType::IN_INACTIVE_MAP:
            ActivePiorityTypeMod = 0.5;
            break;
        case ActivePiorityType::IN_EMPTY_SERVER:
            ActivePiorityTypeMod = 0.4;
            break;
        default:
            ActivePiorityTypeMod = 0.5;
            break;
    ...
    if (maxDiff > 250)
        return ActivePiorityTypeMod * (botActiveAlonePerc * 2) / 10;
    if (maxDiff > 220)
        return ActivePiorityTypeMod * (botActiveAlonePerc * 3) / 10;
    if (maxDiff > 190)
        return ActivePiorityTypeMod * (botActiveAlonePerc * 4) / 10;
    if (maxDiff > 160)
        return ActivePiorityTypeMod * (botActiveAlonePerc * 5) / 10;
    if (maxDiff > 130)
        return ActivePiorityTypeMod * (botActiveAlonePerc * 6) / 10;
    if (maxDiff > 100)
        return ActivePiorityTypeMod * (botActiveAlonePerc * 8) / 10;
    if (maxDiff > 80)
        return ActivePiorityTypeMod * (botActiveAlonePerc * 9) / 10;
