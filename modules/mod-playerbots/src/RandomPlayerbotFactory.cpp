...
int getRandomElement(const std::vector<int>& elements, std::mt19937& gen, std::uniform_int_distribution<>& dist)
{
...
void RandomPlayerbotFactory::CreateRandomBots()
{
    ...
    for (uint32 accountNumber = 0; accountNumber < sPlayerbotAIConfig->randomBotAccountCount; ++accountNumber)
    {
        ...
        for (uint8 cls = CLASS_WARRIOR; cls < MAX_CLASSES - count; ++cls)
        {
            ...
            {
                // 提高圣骑、牧师、萨满和德的概率
                if ((cls == 1 || cls == 3 || cls == 4 || cls == 6 || cls == 8 || cls == 9) && urand(0, 1))
                {
                    std::vector<int> numbers = {2, 5, 7, 11};
                    std::random_device rd;
                    std::mt19937 gen(rd());
                    std::uniform_int_distribution<> dist(0, numbers.size() - 1);
                    cls = getRandomElement(numbers, gen, dist);
                }
